...
<div class="user_info" style="background-color: #377e84; height: 200px">
    ...
    $check_points = mysqli_query($connect, "SELECT * FROM `lesson` WHERE `LessonId` = 1");
    $all_points = (mysqli_fetch_assoc($check_points))['MaxPoint'];
    $right_points = $_SESSION['point'];
    $a = mysqli_query($connect, "UPDATE `session` SET `FinishTime` = '$date', `Points` = $right_points, `Status` = 'Finished' WHERE `LessonId` = 1 AND `UserId` = $user_id and `Status` = 'Started'");
    ?>
    <div class="results">
        <div class="les_end">
            <h2>Урок завершен!</h2>
        ...
        <div class="res_points">
            <h3>Вы ответили правильно на <?php echo $right_points ?> из <?php echo $all_points ?> вопросов</h3>
        </div>
        <div class="next_les">
            <a class="arrow-3" href="../<?php echo $less_id+1 ?>/index.php ">Перейти к следующему уроку
